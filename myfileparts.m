function [path, name, ext] = myfileparts(file,whatpiece,forceispc)

% based on MATLABs fileparts. but does not split NAME and EXT if only two
% outputs requested.
%
% [pathorfile] = FILEPARTS(FILE,whatpiece,forceispc)
% 
%       whatpiece: string containing any of 'p','f','e' for path, file,
%       extension.
%       forceispc: boolean to force interpreting paths in pc style (filesep = '\')
% 
...
if nargout <= 1 && not(isempty(whatpiece))
    p = path;
    path = '';
    ...
    if not(isempty(strfind(whatpiece,'e')))
        path = [path ext];
    end
end
if nargout > 2
